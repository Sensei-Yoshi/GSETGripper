...
Edit ``IMAGE_PATH`` below, then run:
    ...
    python -m modules.contact_model.extract_object_outline
...
IMAGE_PATH = Path(
    ...
    "data/MatForce/objects/plastic_cup/image.png"
)
...
def _rembg_api():
    """Import the optional segmenter only when image analysis is requested."""
    try:
        from rembg import new_session, remove
    except ImportError as exc:
        raise RuntimeError(
            "Contact-image segmentation requires the project 'segmentation' extra: "
...
def create_rembg_session():
    """Create the configured background-removal session."""
    new_session, _ = _rembg_api()
...
def segment_foreground(image_bgr: np.ndarray, session=None) -> np.ndarray:
    """Return a clean 0/255 foreground mask for a BGR image."""
    ...
    return largest_foreground_component(rgba[:, :, 3])
...
def process_image(image_path: Path, output_dir: Path, session=None) -> list[Path]:
    ...
    mask = segment_foreground(image_bgr, session=session)
    ...
    overlay_path = output_dir / "spline_overlay.png"
    csv_path = output_dir / "spline_points.csv"
    svg_path = output_dir / "spline.svg"
