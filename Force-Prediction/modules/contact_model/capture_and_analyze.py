...
Pad width is constant and cancels. Results are stored with the source image at
``data/<dataset>/objects/<object>/contact_fraction/``.
"""
...
def click_scale(frame: np.ndarray, reference_width_mm: float) -> float | None:
    ...
    if pixels < 5:
        print("clicked points nearly coincide; aborting scale")
        return None
...
def main() -> int:
    ...
    parser.add_argument("--dataset", default="collected")
    ...
    dataset_id = re.sub(r"[^a-z0-9]+", "_", args.dataset.lower()).strip("_")
    if not dataset_id:
        parser.error("dataset must contain at least one letter or number")
    objects_root = args.data_root / dataset_id / "objects"
    ...
    def run_one(frame: np.ndarray | None, image_source: Path | None) -> None:
        ...
        name = re.sub(r"[^a-z0-9]+", "_", name.lower()).strip("_") or "object"
        object_dir = objects_root / name
        if object_dir.exists():
            raise FileExistsError(
                f"object {name!r} already exists in dataset {dataset_id!r}"
            )
        image = frame if frame is not None else cv2.imread(str(image_source))
        if image is None:
            raise SystemExit(f"could not read {image_source}")

        if args.px_per_mm is None:
            args.px_per_mm = click_scale(image, args.ref_width_mm)
            if args.px_per_mm is None:
                print("no scale set; capture discarded")
                return
        ...
        with tempfile.TemporaryDirectory(
            ...
            image_path = staged_object / "image.png"
        ...
        if not args.no_show:
            show_image(object_dir / "contact_fraction" / "contact.png")
